	...
	export default {
		...
		mounted() {
			...
			if(this.$route.params.pok>0){
				 $.ajax({url: apiurl.LoginByOpenId,data:{OpenId:this.openid},type:'get',success:function(data){
					console.log(vm.$route.params.pok,data)
					$.ajax({ url: apiurl.GetPatientByPatientID+"?PatientId="+vm.$route.params.pok,headers:headers, success: function(data){
						//console.log(data.extra.PatientIdCardPhotoUrl)
				        if(data.suc){
				        	console.log(vm.pictureurl)
				        	if(vm.pictureurl==""){
				        	...
				        	// $(".imgList img").eq(0).attr("src",imgurl[0]);
				        	// $(".imgList img").eq(0).attr("data-val",0);
				        	// $(".imgList img").eq(1).attr("src",imgurl[1]);
				        	// $(".imgList img").eq(1).attr("data-val",1);
				        	// $(".imgList img").eq(2).attr("src",imgurl[2]);
				        	// $(".imgList img").eq(2).attr("data-val",2);
				        	// $('.playok_btn').addClass('displaycolor');
		...
		methods: {
			...
			gophotograph(value) {
				...
				this.$router.push({'name':"Photographchange"});
			},
			playokSubmit(){
				...
				// if(this.$route.params.pok>0){
					vm.imgs.push((vm.imgtureurl==undefined) ? vm.pictureurl : vm.imgtureurl);
					vm.imgs.push((vm.imgtureurlt==undefined) ? vm.pictureurlt : vm.imgtureurlt);
					vm.imgs.push((vm.imgtureurltr==undefined) ? vm.pictureurltr : vm.imgtureurltr);
				// }else{
				...
				$.ajax({
	  			...
	  			success:function(data){
	  				console.log(data.rows[0].PatientIdCardPhotoUrlList,"ovr")
	  				Indicator.close();
	  				if(data.suc){
		  				...
		  				vm.$router.push({'name':"successFailchange",params: { succ:vm.$route.params.pok}});
		  			}else{
		  				vm.set_picbackimgs("");
	  					vm.set_picimgs("");
